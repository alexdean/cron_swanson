...
RSpec.describe CronSwanson do
  ...
  describe '.schedule' do
    it 'raises an error if interval is greater than 24 hours' do
      expect {
        CronSwanson.schedule('test_job', interval: CronSwanson::SECONDS_PER_DAY * 2)
    ...
    it 'raises an error if interval is not a factor of 24 hours' do
      expect {
        CronSwanson.schedule('test_job', interval: 61)
    ...
    describe 'without a specified interval' do
      it 'creates a cron-compatible schedule string for a daily job' do
        expect(CronSwanson.schedule('test_job')).to eq "52 4 * * *"
      end
    end

    describe 'with a specified interval' do
      it 'creates cron-compatible schedule strings for the specified intervals' do
        six_hours = 60 * 60 * 6
        expect(CronSwanson.schedule('test_job',       interval: six_hours)).to eq "13 1,7,13,19 * * *" # rubocop:disable Metrics/LineLength
        expect(CronSwanson.schedule('other_test_job', interval: six_hours)).to eq "24 0,6,12,18 * * *" # rubocop:disable Metrics/LineLength

        twelve_hours = 60 * 60 * 12
        expect(CronSwanson.schedule('test_job',       interval: twelve_hours)).to eq "26 2,14 * * *"
        expect(CronSwanson.schedule('other_test_job', interval: twelve_hours)).to eq "48 0,12 * * *"
      end
    end
  end
end
